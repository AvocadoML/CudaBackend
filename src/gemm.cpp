				...
				const avTensorDescriptor_t cDesc, avMemoryDescriptor_t cMem)
		{
			...
			switch (getTensor(cDesc).dtype())
			{
				case AVOCADO_DTYPE_INT32: // AB [int8], C [int32]
				{
					...
					cublasStatus_t status = cublasGemmEx(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer(bMem), CUDA_R_8I,
							LDB, getPointer(aMem), CUDA_R_8I, LDA, &_beta, getPointer(cMem), CUDA_R_32I, LDC, CUDA_R_32I, CUBLAS_GEMM_DEFAULT);
...
//					cublasStatus_t status = cublasGemmEx(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer(bMem),
//							CUDA_R_16BF, LDB, getPointer(aMem), CUDA_R_16BF, LDA, &_beta, getPointer(cMem), CUDA_R_16BF, LDC, CUDA_R_32F,
//							CUBLAS_GEMM_DEFAULT);
//					return convertStatus(status);
//				}
				case AVOCADO_DTYPE_FLOAT16: // ABC [float16]
				{
					...
					if (sm_ver == 53 or sm_ver == 60 or sm_ver >= 62)
					{
						...
						cublasStatus_t status = cublasHgemm(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer<half>(bMem),
								LDB, getPointer<half>(aMem), LDA, &_beta, getPointer<half>(cMem), LDC);
						return convertStatus(status);
					}
					else
					{
						...
						float _beta = getBetaValue(beta);
						cublasStatus_t status = cublasGemmEx(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer(bMem),
								CUDA_R_16F, LDB, getPointer(aMem), CUDA_R_16F, LDA, &_beta, getPointer(cMem), CUDA_R_16F, LDC, CUDA_R_32F,
								CUBLAS_GEMM_DEFAULT);
						return convertStatus(status);
					}
				}
				...
				case AVOCADO_DTYPE_FLOAT64: // ABC [float64]
				{
					...
					cublasStatus_t status = cublasDgemm(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer<double>(bMem),
							LDB, getPointer<double>(aMem), LDA, &_beta, getPointer<double>(cMem), LDC);
					return convertStatus(status);
				}
				case AVOCADO_DTYPE_COMPLEX32: // ABC [complex32]
				{
					...
					cublasStatus_t status = cublasCgemm(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha, getPointer<cuComplex>(bMem),
							LDB, getPointer<cuComplex>(aMem), LDA, &_beta, getPointer<cuComplex>(cMem), LDC);
					return convertStatus(status);
				}
				case AVOCADO_DTYPE_COMPLEX64: // ABC [complex64]
				{
					...
					cublasStatus_t status = cublasZgemm(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
							getPointer<cuDoubleComplex>(bMem), LDB, getPointer<cuDoubleComplex>(aMem), LDA, &_beta,
							getPointer<cuDoubleComplex>(cMem), LDC);
					return convertStatus(status);
				}
				default:
					return AVOCADO_STATUS_UNSUPPORTED_DATATYPE;
			...
			switch (getTensor(cDesc).dtype())
			{
				case AVOCADO_DTYPE_FLOAT16:
				{
					...
					if (sm_ver == 53 or sm_ver == 60 or sm_ver >= 62)
					{
						half _alpha = getAlphaValue(alpha);
						half _beta = getBetaValue(beta);
						cublasStatus_t status = cublasHgemmStridedBatched(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
								getPointer<half>(bMem), LDB, strideB, getPointer<half>(aMem), LDA, strideA, &_beta, getPointer<half>(cMem),
								LDC, strideC, batch);
						return convertStatus(status);
					}
					else
					{
						float _alpha = getAlphaValue(alpha);
						float _beta = getBetaValue(beta);
						cublasStatus_t status = cublasGemmStridedBatchedEx(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
								getPointer(bMem), CUDA_R_16F, LDB, strideB, getPointer(aMem), CUDA_R_16F, LDA, strideA, &_beta,
								getPointer(cMem), CUDA_R_16F, LDC, strideC, batch, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
						return convertStatus(status);
					}
				}
				case AVOCADO_DTYPE_FLOAT32:
				{
					float _alpha = getAlphaValue(alpha);
					float _beta = getBetaValue(beta);
					cublasStatus_t status = cublasSgemmStridedBatched(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
							getPointer<float>(bMem), LDB, strideB, getPointer<float>(aMem), LDA, strideA, &_beta, getPointer<float>(cMem),
							LDC, strideC, batch);
					return convertStatus(status);
				}
				case AVOCADO_DTYPE_FLOAT64:
				{
					double _alpha = getAlphaValue<double>(alpha);
					double _beta = getBetaValue<double>(beta);
					cublasStatus_t status = cublasDgemmStridedBatched(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
							getPointer<double>(bMem), LDB, strideB, getPointer<double>(aMem), LDA, strideA, &_beta, getPointer<double>(cMem),
							LDC, strideC, batch);
					return convertStatus(status);
				}
				case AVOCADO_DTYPE_COMPLEX32:
				{
					...
					cublasStatus_t status = cublasCgemmStridedBatched(getContext(context).getHandle(), op_B, op_A, M, N, K, &_alpha,
							getPointer<cuComplex>(bMem), LDB, strideB, getPointer<cuComplex>(aMem), LDA, strideA, &_beta,
							getPointer<cuComplex>(cMem), LDC, strideC, batch);
					return convertStatus(status);
				}
