			...
			switch (getTensor(xDesc).dtype())
			{
				...
				case AVOCADO_DTYPE_FLOAT64:
				{
					helper_batchnorm_backward(getContext(context), activation, getAlphaValue<double>(alpha), getTensor(xDesc), getPointer<double>(xMem),
							...
							getTensor(dyDesc), getPointer<double>(dyMem), getTensor(scaleMeanVarDesc), getPointer<double>(scaleMem), getPointer<double>(meanMem),
							getPointer<double>(varianceMem), getAlphaValue<double>(alpha2), getBetaValue<double>(beta2), getPointer<double>(scaleUpdateMem),
							getPointer<double>(biasUpdateMem), epsilon);
					break;
				}
				default:
					return AVOCADO_STATUS_UNSUPPORTED_DATATYPE;
